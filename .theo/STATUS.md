...
- **Done this session:**
  ...
  - **Portrait Display & Game Calibration:** Resolved critical database insert bug where the `portrait` field was dropped by the Hono API upon character creation (POST `/api/characters`). Fixed player HUD StatusTab, InspectModal, and SavedCrawlerLibrary to cleanly render dynamic custom portraits from character database strings rather than falling back to name-based arrays. Mapped the character Level and Class display to the **Desktop Header** (previously only visible on Mobile layouts) so it displays correctly across all screen sizes, resolved a hardcoded \"LVL 2\" string in the Saved Crawler Reconstruction Library card, and corrected character leveling defaults to 1.
