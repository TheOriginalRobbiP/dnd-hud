...
- **Done this session**:
  ...
  - **Click-to-Load Dynamic Action Roller**: Integrated a central `selectedAction` state in `PlayerHUD.tsx` and propagated it to `DiceHero.tsx`, `SkillsTab.tsx`, and `InventoryTab.tsx`. Clicking any skill, active spell, equipped item, or backpack item now instantly loads it as the active action in the Dice Roller (`DiceHero`), configuring its name, modifiers, and effort dice dynamically. Includes an on-demand "✕ RESET" clear action on the roller card.
